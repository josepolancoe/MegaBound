...
module.exports = class Room {
    ...
    gameStart(account) {
        ...
        if (self.status === Types.ROOM_STATUS.WAITING || self.status === Types.ROOM_STATUS.FULL) {
            if ((account.player.gm === 1) || ((self.player_count > 0) && self.game_mode === Types.GAME_MODE.BOSS && self.team_bots_count>0) || ((self.player_count > 1) && self.game_mode === Types.GAME_MODE.NORMAL && self.team_a_count === self.team_b_count)) {
                ...
                account.sendMessage(new Message.alertResponse(":(", "Usted no puede jugar sólo!"));
            }
        }
    }
