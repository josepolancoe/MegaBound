...
module.exports = class GameServer {
  constructor(id, maxPlayers, websocketserver) {
    ...
    this.onAccountEnter(function (account) {
      //acciones de inicio del servidor
      //account.sendMessage(new Message.alertResponse(":(", "Bienvenido!"));
      if (self.chathistory !== null) {
        var data = self.chathistory.slice(0);
        data.push(["", "", 9]);
        if (self.evento200) {
          data.push(["Evento 500% GP 24h - Desarrollado por Andres Polanco", "", 6]);
          //1 Regalo, 3 Gold in game, 4 is Power in chat, 5 Chat GM, 6 System
          data.push(["La Familia MegaBound te invita su grupo en FB => https://cutt.ly/grupo-mega-bound", "", 6]);
        }
