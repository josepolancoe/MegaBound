...
module.exports = class DataBase {
    ...
    getUserAndGuildByGameId(game_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
    ...
    getUserGuild(guild_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT u.Id, u.IdAcc, u.game_id, u.rank, u.gp, u.gender, gm.Id, gm.UserId, gm.Job, ac.Session, g.Id as id_guild, g.Name FROM users u INNER JOIN accounts ac ON u.IdAcc = ac.Id LEFT JOIN guild_member gm ON gm.UserId = ac.Id LEFT JOIN guild g on gm.Id = g.Id WHERE g.Name = ?', [guild_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
